...
echo "$PROMPT bash run-masscan.sh $CLIENT ~/$CLIENT/targets 2500 | tee ~/$CLIENT/all.masscan"
bash run-masscan.sh $CLIENT ~/$CLIENT/targets 2500 | tee ~/$CLIENT/all.masscan
...
for IP in $(grep 500 ~/$CLIENT/nmap-known-udp-ports-2.gnmap|grep Host:|egrep -o '([0-9]{1,3}\.){3}([0-9]{1,3})'|egrep -v '^$'); do
