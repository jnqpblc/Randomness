...
FILE=$1; CLIENT=$2; for HOST in $(awk '{print $6}' $FILE|sort -u); do
   ...
   echo "sudo nmap -PN -sSV -p $PORTS --script 'ALL and not (broadcast or dos or fuzzer)' --script-args $uag --script-args $apikey --open -oA ~/$CLIENT/nmap-output-$HOST $HOST"
   ...
   sudo nmap -PN -sSV -p $PORTS --open -oA ~/$CLIENT/nmap-output-$HOST $HOST
   done
fi
