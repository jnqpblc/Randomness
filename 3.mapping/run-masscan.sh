...
if [ -z $3 ]; then printf "\nSytnax: $0 <client> <file of ips> <rate|e.g. 1000>\n\n"
   ...
   echo "~/masscan/bin/masscan -iL $FILE -p T:0-65535 --rate $RATE --banners -oB ~/$CLIENT/masscan-output.bin --interface eth0"
   sudo ~/masscan/bin/masscan -iL $FILE -p T:0-65535 --rate $RATE --banners -oB ~/$CLIENT/masscan-output.bin --interface eth0|tee ~/$CLIENT/masscan-output.txt
